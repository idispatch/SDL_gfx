 ...
 */
static unsigned int GFX_ALPHA_ADJUST_ARRAY[256] = { 0, /* 0 */
15, /* 1 */
22, /* 2 */
27, /* 3 */
31, /* 4 */
...
void _SDL_gfxBlitBlitterRGBA(SDL_gfxBlitInfo * info) {
    ...
    while (height--) {
        GFX_DUFFS_LOOP4(
                { Uint32 pixel; unsigned sR; unsigned sG; unsigned sB; unsigned sA; unsigned dR; unsigned dG; unsigned dB; unsigned dA; unsigned sAA; GFX_DISASSEMBLE_RGBA(src, srcbpp, srcfmt, pixel, sR, sG, sB, sA); GFX_DISASSEMBLE_RGBA(dst, dstbpp, dstfmt, pixel, dR, dG, dB, dA); sAA=GFX_ALPHA_ADJUST_ARRAY[sA & 255]; GFX_ALPHA_BLEND(sR, sG, sB, sAA, dR, dG, dB); dA |= sAA; GFX_ASSEMBLE_RGBA(dst, dstbpp, dstfmt, dR, dG, dB, dA); src += srcbpp; dst += dstbpp; },
                width);
        src += srcskip;
        dst += dstskip;
    }
 ...
 */
int _SDL_gfxBlitRGBACall(SDL_Surface * src, SDL_Rect * srcrect, SDL_Surface * dst,
        SDL_Rect * dstrect) {
    /*
...
int SDL_gfxSetAlpha(SDL_Surface *src, Uint8 a) {
    ...
    if ((src == NULL) || (src->format == NULL) || (src->format->BytesPerPixel != 4)) {
